// Authors: Oliver Sjostrom, Nabor Rios
// Net ID: oliversjost, riosnabor
// Date: 5/10/2023
...
void Read_from(unsigned char SLA, unsigned char MEMADDRESS){
  ...
  TWCR = (1<<TWINT)|(1<<TWSTA)|(1<<TWEN); // Clear TWINT, initiate start condition, initiate enable, switch to read mode
  wait_for_completion;
...
int Read_from_16bit(unsigned char SLA, unsigned char MEMADDRESS){
  ...
  TWCR = (1<< TWINT) | (1 << TWEN) | (1 << TWEA);  // Trigger action + ACK
  wait_for_completion;
  //Serial.println("Trigger Finish");
